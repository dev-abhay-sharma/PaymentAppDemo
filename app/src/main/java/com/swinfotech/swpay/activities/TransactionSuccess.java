...
import com.swinfotech.swpay.databinding.ActivityTransactionSuccessBinding;
...
public class TransactionSuccess extends AppCompatActivity {
    ...
    TransactionSuccess activity;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityTransactionSuccessBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        activity = this;

        if (getIntent() != null) {
            SearchByNumberGetSet data = (SearchByNumberGetSet) getIntent().getSerializableExtra("data");
            // check if data not null
            if (data != null) {
                binding.successAmount.setText(getAmountFormat(data.getAmount()));
                binding.successStatus.setText(data.getStatus());
                if (data.getStatus().equalsIgnoreCase("FAILED")) {
                    binding.successStatus.setTextColor(ContextCompat.getColor(this, R.color.red));
                    binding.successImg.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.failed));
                } else {
                    binding.successImg.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.success));
                    binding.successStatus.setTextColor(ContextCompat.getColor(this, R.color.green));
                }

                binding.successOrId.setText("Order Id " + data.getTrId());

                binding.successMob.setText("Phone " + data.getPhNum());
                binding.successOperator.setText(data.getOpName());
//                binding.successDate.setText(data.getDate());
                binding.successDate.setText(getDateAndMonth(data.getDate())+" " + getTime(data.getDate()));
                ...
                if (image.substring(image.lastIndexOf(".") + 1).equalsIgnoreCase("svg")) {
                    ...
                    GlideToVectorYou
                            ...
                            .load(uri, binding.successLogo1);

                } else {
                    Glide.with(activity)
                            .load(Constant.COMMISSION_IMAGES_BASE_URL + data.getImage())
                            .into(binding.successLogo1);
                }
...
//        binding.complain1.setOnClickListener(v -> {
//            startActivity(new Intent(activity, RaiseComplaint.class));
//        });

        binding.share1.setOnClickListener(v -> {
            if (CheckPermission()) {
                shareImage(getMainFrameBitmap(findViewById(R.id.rootView1)));
            }
        });
