...
import com.swinfotech.swpay.databinding.ActivityTransactionReportSuccessBinding;
...
public class TransactionReportSuccess extends AppCompatActivity {

    ActivityTransactionReportSuccessBinding binding;

    TransactionReportSuccess activity;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityTransactionReportSuccessBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        activity = this;

        if (getIntent() != null) {
            TransactionReportGetSet data = (TransactionReportGetSet) getIntent().getSerializableExtra("data");

            // check if data not null
            if (data != null) {
//                binding.successAmount1.setText(data.getAmount());
                binding.successAmount1.setText(getAmountFormat(data.getAmount()));
//                binding.successLogo.setImageResource(Integer.parseInt(data.getOpImage()));
                binding.successStatus1.setText(data.getStatus());
                binding.successApiRefId.setText(data.getApiRef());
                if (data.getStatus().equalsIgnoreCase("FAILED")) {
                    ...
                    binding.successStatus1.setTextColor(ContextCompat.getColor(this, R.color.yellow));
                    binding.successImg1.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.failed));
                } else if (data.getStatus().equalsIgnoreCase("REFUND")) {
                    binding.successStatus1.setTextColor(ContextCompat.getColor(this, R.color.red));
                    binding.successImg1.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.failed));
                } else {
                ...
                binding.successOrId1.setText("Order Id : " + data.getTrId());

                binding.successMob1.setText(data.getRechargeNum());
                binding.successOperator1.setText(data.getOpName());
                binding.successDate1.setText(data.getTrDate());

                String image = data.getOpImage();

                // check if image is svg or not
                if (image.substring(image.lastIndexOf(".") + 1).equalsIgnoreCase("svg")) {
                    // make uri from url
                    Uri uri = Uri.parse(Constant.COMMISSION_IMAGES_BASE_URL + data.getOpImage());
                    ...
                    GlideToVectorYou
                            ...
                            .load(uri, binding.successLogo);

                } else {
                    Glide.with(activity)
                            .load(Constant.COMMISSION_IMAGES_BASE_URL + data.getOpImage())
                            .into(binding.successLogo);
                }
//                binding.successDate1.setText(getDateAndMonth(data.getTrDate())+" " + getTime(data.getTrDate()));
            } else {
        ...
        TransactionReportGetSet data1 = (TransactionReportGetSet) getIntent().getSerializableExtra("data");

        String status = data1.getStatus();
        if (status.equalsIgnoreCase("SUCCESS")) {
            binding.raiseComp.setOnClickListener(v -> {
//            startActivity(new Intent(activity, RaiseComplaint.class));
                Intent intent = new Intent(activity, RaiseComplaintTransaction.class);
                intent.putExtra("tr_id", data1.getTrId());
                intent.putExtra("amount", data1.getAmount());
                startActivity(intent);

            });
        } else {
            binding.raiseComp.setVisibility(View.GONE);
        }
...
//        });
        ...
        binding.share.setOnClickListener(v -> {
            if (CheckPermission()) {
                shareImage(getMainFrameBitmap(findViewById(R.id.rootView)));
            }
        });
